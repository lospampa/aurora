...
{
//  printf("gomp_resolve_num_threads -- parallel.c\n");
...
{
//  printf("gomp_cancellation_point -- parallel.c\n");
  if (!gomp_cancel_var)
    return false;
...
{
//  printf("omp_get_num_threads -- parallel.c\n");
...
{
//  printf("omp_get_thread_num -- parallel.c\n");
...
{
//  printf("omp_in_parallel -- parallel.c\n");
...
omp_get_level (void)
{
//  printf("omp_get_level -- parallel.c\n");
...
{
//  printf("omp_get_ancestor_thread_num -- parallel.c\n");
...
{
//  printf("omp_get_team_size -- parallel.c\n");
  struct gomp_team_state *ts = &gomp_thread ()->ts;
  if (level < 0 || level > ts->level)
    return -1;
  for (level = ts->level - level; level > 0; --level)
...
{
//  printf("omp_get_active_level -- parallel.c\n");
